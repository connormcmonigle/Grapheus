...
struct SeerModel : ChessModel {
    SparseInput* in1;
    SparseInput* in2;

    static constexpr float  sigmoid_scale = 1.0 / 1024.0;
    static constexpr float  quant_one     = 512.0;
    static constexpr float  quant_two     = 1024.0;

    static constexpr size_t n_features    = 64 * 12 * 64;
    static constexpr size_t n_l1          = 8;
    static constexpr size_t n_l2          = 8;
    static constexpr size_t n_l3          = 8;

    static constexpr size_t n_out         = 1;

    SeerModel(size_t n_ft, float lambda, size_t save_rate)
        : ChessModel(lambda) {
        ...
        auto        l1         = add<Affine>(fta, n_l1);
        auto        l1a        = add<ReLU>(l1);
        auto        l1ac       = add<Linear>(l1a);        

        auto        l2         = add<Affine>(l1a, n_l2);
        auto        l2a        = add<ReLU>(l2);
        auto        l2m        = add<Merge>(l1ac, l2a);
        auto        l2mc       = add<Linear>(l2m);

        auto        l3         = add<Affine>(l2m, n_l3);
        auto        l3a        = add<ReLU>(l3);
        auto        l3m        = add<Merge>(l2mc, l3a);


        auto        pos_eval   = add<Affine>(l3m, n_out);
        auto        sigmoid    = add<Sigmoid>(pos_eval);
        ...
        add_quantization(Quantizer {
            ...
            QuantizerEntry<int16_t>(&ft->weights.values, quant_one, true),
            QuantizerEntry<int16_t>(&ft->bias.values, quant_one),
            QuantizerEntry<int16_t>(&l1->weights.values, quant_two),
            QuantizerEntry<int32_t>(&l1->bias.values, quant_one * quant_two),
            QuantizerEntry<float>(&l2->weights.values, 1.0),
            QuantizerEntry<float>(&l2->bias.values, 1.0),
            QuantizerEntry<float>(&pos_eval->weights.values, 1.0),
            QuantizerEntry<float>(&pos_eval->bias.values, 1.0),
        });
    }
    ...
    inline int index(chess::Square piece_square,
        ...
        const int oP  = piece_type + 6 * (piece_color != view);
        const int oK  = (56 * view) ^ king_square;
        const int oSq = (56 * view) ^ piece_square;

        return oK * 12 * 64 + oP * 64 + oSq;
    }
